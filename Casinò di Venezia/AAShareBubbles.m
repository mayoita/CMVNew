...
- (id)initWithFrame:(CGRect)rect andButtonFrame:(CGRect)buttonRect radius:(int)radiusValue inView:(UIView *)inView
{
   
    self = [super initWithFrame:CGRectMake(0, 0, radiusValue*2, radiusValue*2)];
    self.center=CGPointMake(rect.size.width/2, rect.size.height/2);
    self.homePoint=CGPointMake(buttonRect.origin.x + buttonRect.size.width/2-rect.size.width/2+radiusValue, buttonRect.origin.y + buttonRect.size.height/2-rect.size.height/2+radiusValue);
...
-(void)show
{
    ...
    if(!self.isAnimating)
    {
        ...
        if(self.showFacebookBubble)     [self createButtonWithIcon:@"FacebookIcon.png" backgroundColor:self.facebookBackgroundColorRGB andType:AAShareBubbleTypeFacebook];
        if(self.showTwitterBubble)      [self createButtonWithIcon:@"TwitterIcon.png" backgroundColor:self.twitterBackgroundColorRGB andType:AAShareBubbleTypeTwitter];
        ...
        if(self.showVkBubble)           [self createButtonWithIcon:@"CalendarIcon.png" backgroundColor:self.vkBackgroundColorRGB andType:AAShareBubbleTypeVk];
        ...
        for (int i = 0; i < bubbles.count; ++i)
        {
            ...
            bubble.center = CGPointMake(self.homePoint.x, self.homePoint.y);
        }
